...
const wallModeConfig = {
  up: {
    icon: (props: any) => (
      <img alt="Altura completa" height={28} src={assetPath("/icons/room.png")} width={28} {...props} />
    ),
    label: 'Altura completa',
  },
  cutaway: {
    icon: (props: any) => (
      <img alt="Corte" height={28} src={assetPath("/icons/wallcut.png")} width={28} {...props} />
    ),
    label: 'Corte',
  },
  down: {
    icon: (props: any) => (
      <img alt="Bajo" height={28} src={assetPath("/icons/walllow.png")} width={28} {...props} />
    ),
    label: 'Bajo',
  },
}
...
const getNodeName = (node: AnyNode): string => {
  ...
  if (node.type === 'item') return (node as { asset: { name: string } }).asset?.name || 'Elemento'
  if (node.type === 'slab') return 'Losa'
  if (node.type === 'ceiling') return 'Cielorraso'
  if (node.type === 'roof') return 'Techo'
  if (node.type === 'roof-segment') return 'Segmento de techo'
  return node.type
}
          ...
          <div className="pointer-events-auto flex h-14 flex-row items-center justify-center gap-1.5 rounded-2xl border border-border/40 bg-background/95 p-1.5 shadow-lg backdrop-blur-xl transition-colors duration-200 ease-out">
            ...
            {canShowScans && (
              <ActionButton
                ...
                label={`Scans: ${showScans ? 'Visible' : 'Oculto'}`}
            ...
            {canShowGuides && (
              <ActionButton
                ...
                label={`Planos: ${showGuides ? 'Visible' : 'Oculto'}`}
